...
async function scrapeCompanies() {
    ...
    try {
    await page.waitForSelector('[id="results"]',{timeout:1000});
        } catch (error) {
    ...
    const companyLinks = await page.evaluate(() => {
      //  return [...document.querySelectorAll('.search-result-item a')]
        return [...document.querySelectorAll("#results > table > tbody > tr > td:nth-child(1) > a")]
            .map(link => link.href)
    ...
    for (let link of companyLinks) {
        ...
        try {
            ...
            const companyDetails = await page.evaluate(() => {
                ...
                const address = document.querySelector('[class="Adresse_actionWrapper__R83lq"] > a').textContent.trim() || 'N/A';
